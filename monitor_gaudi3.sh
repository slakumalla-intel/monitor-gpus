...
QUERY_PARAMS2="timestamp,name,bus_id,ecc.errors.uncorrected.aggregate.total,ecc.errors.uncorrected.volatile.total,ecc.errors.corrected.aggregate.total,ecc.errors.corrected.volatile.total,ecc.errors.dram.aggregate.total,ecc.errors.dram-corrected.aggregate.total,ecc.errors.dram.volatile.total,ecc.errors.dram-corrected.volatile.total,ecc.mode.current,ecc.mode.pending"
QUERY_PARAMS3="timestamp,name,bus_id,stats.violation.power,stats.violation.thermal,clocks.current.soc,clocks.max.soc,clocks.limit.soc,clocks.limit.tpc,pcie.link.gen.max,pcie.link.gen.current,pcie.link.width.max,pcie.link.width.current,pcie.link.speed.max,pcie.link.speed.current"
...
monitor_gpu() {
   ...
   # HOST=$(hostname)
   # echo "$HOST: 0000:$BUS_ID" >> $LOG_FILE4
   # hl-smi -n ports -i 0000:$BUS_ID >> $LOG_FILE4
   # hl-smi -n link -i 0000:$BUS_ID >> $LOG_FILE4
   # data=$(/opt/habanalabs/qual/gaudi3/bin/manage_network_ifs.sh --status)
   # echo $data >> $LOG_FILE4
   # hl-smi -n stats -i 0000:$BUS_ID >> $LOG_FILE4
    ...
    echo "Monitoring GPU with bus ID: $BUS_ID" 
        hl-smi --query-aip=$QUERY_PARAMS1 --format=$FORMAT -i 0000:$BUS_ID >> $LOG_FILE1 #TODO busid
        hl-smi --query-aip=$QUERY_PARAMS2 --format=$FORMAT -i 0000:$BUS_ID >> $LOG_FILE2
        hl-smi --query-aip=$QUERY_PARAMS3 --format=$FORMAT -i 0000:$BUS_ID >> $LOG_FILE3
        sleep $PERIODIC_TIME
    done
}
...
echo "$BUS_IDS" | xargs -I {} -n 1 -P 8 bash -c 'monitor_gpu "{}" "$@"' _ "$QUERY_PARAMS1" "$QUERY_PARAMS2" "$QUERY_PARAMS3" "$FORMAT2" "$LOG_FILE1" "$LOG_FILE2" "$LOG_FILE3" "$PERIODIC_TIME" 
